...
public class BalancedStick implements Mobile // TODO: activate clause.
{
    ...
    private Mobile[] attached = new Mobile[2];
    ...
    public BalancedStick(int stickWeight, Mobile left, Mobile right) throws UnbalancedException {
        ...
        if (left.getWeight() != right.getWeight())
            ...
            this.attached[0] = left;
            this.attached[1] = right;
        }
    }
    ...
    public boolean replace(Mobile toReplace, Mobile replaceWith) throws UnbalancedException {
        if (!attached[0].equals(toReplace) && !attached[1].equals(toReplace)) return false;
        if (toReplace.getWeight() != replaceWith.getWeight())
            throw new UnbalancedException("Stick unbalanced (left " + toReplace.getWeight() + " - right " + replaceWith.getWeight() + ")");
        int i = attached[0].equals(toReplace) ? 0 : 1;
    ...
    public int getWeight() {
        int size = stickWeight;
        for (Mobile mobile : attached) {
            size += mobile.getWeight();
        }
        return size;
    }
    ...
    public boolean equals(Object o) {
        ...
        if (this.attached[0].hashCode() == that.attached[0].hashCode()) {
            return this.attached[1].hashCode() == that.attached[1].hashCode();
        }

        if (this.attached[0].hashCode() == that.attached[1].hashCode())
            return this.attached[1].hashCode() == that.attached[0].hashCode();
    ...
    public int hashCode() {
        ...
        return result * this.stickWeight;
    }

    @Override
    public StarIterator iterator() {
        return new StickIter(attached);
    }

    @Override
    public StarCollection getStarCollection() {
        return new StickCol(attached, this);
    }

    @Override
    public String toString() {
        String left = "(" + attached[0].toString() + ")";
        String self = "-" + this.stickWeight + "-";
        String right = "(" + attached[1].toString() + ")";
