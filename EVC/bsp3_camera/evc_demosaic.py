...
def evc_transform_neutral(R: np.ndarray, G: np.ndarray, B: np.ndarray, asShotNeutral: np.ndarray) -> Tuple[
...
def evc_interpolate(red: np.ndarray, green: np.ndarray, blue: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    R_filter = np.array(([[0.25, 0.5, 0.25], [0.5, 1, 0.5], [0.25, 0.5, 0.25]]))
    G_filter = np.array(([[0, 0.25, 0], [0.25, 1, 0.25], [0, 0.25, 0]]))
    B_filter = R_filter

    R_inter = scipy.ndimage.correlate(red, R_filter, mode='constant')
    G_inter = scipy.ndimage.correlate(green, G_filter, mode='constant')
    B_inter = scipy.ndimage.correlate(blue, B_filter, mode='constant')
    ### END STUDENT CODE
