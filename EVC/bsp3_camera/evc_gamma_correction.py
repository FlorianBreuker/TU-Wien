...
def evc_compute_brightness(input_image: np.ndarray) -> np.ndarray:
    ...
    max_color = np.max(input_image)
    input_image *= 1 / max_color
    brightness = rgb2gray(input_image) * max_color
    ### END STUDENT CODE
...
def evc_compute_chromaticity(input_image: np.ndarray, brightness: np.ndarray) -> np.ndarray:
    ...
    #       from crashing.
    chromaticity_r = input_image[:, :, 0] / brightness
    chromaticity_g = input_image[:, :, 1] / brightness
    chromaticity_b = input_image[:, :, 2] / brightness
    chromaticity = np.dstack((chromaticity_r, chromaticity_g, chromaticity_b))
    ### END STUDENT CODE
