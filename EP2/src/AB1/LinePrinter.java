...
public class LinePrinter {
    ...
    public LinePrinter(Font font, int lineLength, int spacing) {
        ...
        createLineBuffer(lineLength, this.font.getHeight(), this.font.getWidth());
    }
    ...
    private void createLineBuffer(int lineLength, int cellHeight, int cellWidth) {
        ...
        int bufferWidth = lineLength * cellWidth + (lineLength - 1) * spacing;
        this.lineBuffer = new char[cellHeight][bufferWidth];

        for (int i = 0; i < this.lineBuffer.length; i++) {
            for (int j = 0; j < this.lineBuffer[i].length; j++) {
                lineBuffer[i][j] = 32;
            }
        }
    }
    ...
    private void printCharacter(char character) {
        // TODO: implementation
        char[][] charBitmap = font.getBitmap(character);

        if (this.cursorPosition + this.font.getWidth() <= this.lineBuffer[0].length) {
            for (int i = 0; i < this.font.getWidth(); i++) {
                for (int j = 0; j < charBitmap.length; j++) {
                    lineBuffer[j][cursorPosition] = charBitmap[j][i];
                }
                cursorPosition++;
            }
            cursorPosition += spacing - 1;
        }
    }
    ...
    public void printString(String string) {
        ...
        for (int i = 0; i < string.length(); i++) {
            printCharacter(string.toLowerCase().charAt(i));
            cursorPosition++;
        }
    }
    ...
    public void flush() {
        ...
        for (int i = 0; i < lineBuffer.length; i++) {
            char[] currentLine = getLineBufferRow(i);
