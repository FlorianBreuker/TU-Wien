...
public class DinosaurHashMapIterator {
    ...
    public DinosaurHashMapIterator(BucketList[] buckets) {
        ...
        if (this.buckets[bucketIndex] == null) {
            setNextNonEmptyBucket();
        }
        else this.currentBucketIterator = buckets[bucketIndex].iterator();
    }
    ...
    private boolean setNextNonEmptyBucket() {
        if (bucketIndex >= buckets.length - 1) return false;
        do {
            if (bucketIndex >= buckets.length - 1) return false;
    ...
    public boolean hasNext() {
        ...
        int i = bucketIndex;
        do {
            if (i >= buckets.length - 1) return false;
    ...
    public Dinosaur next() {
        ...
        Dinosaur dino = currentBucketIterator.next();
        if (dino != null) return dino;
        else if (setNextNonEmptyBucket()) return currentBucketIterator.next();
        else return null;
    }
}
