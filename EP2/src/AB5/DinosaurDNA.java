...
public class DinosaurDNA {
    ...
    public boolean equals(Object obj) {
        if (obj.getClass() == this.getClass()) {
            return ((DinosaurDNA) obj).geneticCode == this.geneticCode;
        }
        return false;
    }
    ...
    public String toString() {
        StringBuilder geneticCodeBinary = new StringBuilder();
        int geneticTemp = this.geneticCode;
        while (geneticTemp != 0) {
            if (geneticTemp % 2 == 0) geneticCodeBinary.insert(0, 0);
            else geneticCodeBinary.insert(0, 1);
            geneticTemp /= 2;
        }

        return String.format("DNA: %s (%s)", geneticCodeBinary, this.geneticCode);
    }
}
