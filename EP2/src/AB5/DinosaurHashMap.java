...
public class DinosaurHashMap implements HashMap {
    ...
    public DinosaurHashMap(int capacity, int maxBucketSize) {
        if (capacity <= 0) this.buckets = new BucketList[DEFAULT_NUM_OF_BUCKETS];
        else this.buckets = new BucketList[DEFAULT_MAX_BUCKET_SIZE];

        if (maxBucketSize <= 0) this.maxBucketSize = DEFAULT_MAX_BUCKET_SIZE;
        else this.maxBucketSize = maxBucketSize;
    ...
    private BucketList[] reorganizeBuckets(int newCapacity) {
        ...
        if (newCapacity <= 0) newBuckets = new BucketList[DEFAULT_NUM_OF_BUCKETS];
        else newBuckets = new BucketList[newCapacity];

        for (int i = 0; i < newBuckets.length; i++) {
            newBuckets[i] = new DinosaurBucketList();
        }

        DinosaurHashMapIterator i = this.iterator();
        Dinosaur dino;
        while (i.hasNext()) {
            dino = i.next();
            int index = Math.abs(dino.getDNA().getGeneticCode()) % newBuckets.length;
            BucketList currentBucket = newBuckets[index];
            currentBucket.store(dino);
        }
        return newBuckets;
    }
    ...
    public Dinosaur put(Dinosaur dinosaur) {
        ...
        int index = Math.abs(dinosaur.getDNA().getGeneticCode()) % buckets.length;
        if (buckets[index] == null) buckets[index] = new DinosaurBucketList();
        BucketList currentBucket = buckets[index];
        Dinosaur oldDino = currentBucket.store(dinosaur);
        if (currentBucket.size() > this.maxBucketSize) {
            this.buckets = reorganizeBuckets(buckets.length * 2);
        }
        return oldDino;
    }
    ...
    public Dinosaur get(DinosaurDNA dna) {
        ...
        int index = Math.abs(dna.getGeneticCode()) % buckets.length;
        if (buckets[index] == null) return null;
        BucketList list = buckets[index];
        return list.find(dna);
    }
    ...
    public int size() {
        int size = 0;
        DinosaurHashMapIterator i = iterator();
        while (i.hasNext()) {
            size++;
            i.next();
        }
