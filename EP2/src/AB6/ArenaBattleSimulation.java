...
public class ArenaBattleSimulation implements BattleSimulation {
    ...
    public Dinosaur executeFight(Dinosaur dinoA, Dinosaur dinoB) {
        ...
        int score = 0;
        FightingBehavior movesA = dinoA.getFightingBehavior();
        FightingBehavior movesB = dinoB.getFightingBehavior();

        for (int i = 0; i < ArenaFightingBehavior.BATTLEPLAN_SIZE; i++) {
            Action actionA = movesA.getPlannedAction(i);
            Action actionB = movesB.getPlannedAction(i);
            int round = executeAction(actionA, actionB);
            logRound(i, actionA, actionB, round);
            score += round;
        }
    ...
    public int executeAction(Action actionA, Action actionB) {
        ...
        return switch (actionA) {
            case NONE -> 1;
            case BITE -> actionB == Action.DODGE ? 1 : -1;
            case TAIL_WHIP -> actionB == Action.BITE ? 1 : -1;
            case DODGE -> actionB == Action.TAIL_WHIP ? 1 : -1;
        };
    }
