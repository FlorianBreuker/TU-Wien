...
public class BrailleListBuffer implements ListBuffer {
    ...
    public String[] renderScanlines(int spacing) {
        ...
        String[] scanLines = new String[list.get(0).getBitmap().length];
        try {
            for (int i = 0; i < scanLines.length; i++) {
                StringBuilder currentRow = new StringBuilder();
                for (int j = 0; j < list.size(); j++) {
                    for (int k = 0; k < list.get(j).getBitmap()[i].length; k++) {
                        currentRow.append(list.get(j).getBitmap()[i][k]);
                    }
                    if (j < list.size() - 1)
                        currentRow.append(String.valueOf(SPACE_SYMBOL).repeat(spacing));
                }
