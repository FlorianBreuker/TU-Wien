...
public class BrailleReader {
    ...
    private char[][] getBrailleChar(int position, int spacing, String[] brailleLine) {
        // TODO: implementation
        if (brailleLine == null) return null;
        int start = position * (WIDTH + spacing);
        if (start >= brailleLine[0].length()) return null;
    ...
    public String translate(String[] brailleLine, char dotSymbol, int spacing) {
        // TODO: implementation
        if (brailleLine == null) return "";
        int lineLengthCheck = brailleLine[0].length() % (WIDTH + spacing);
