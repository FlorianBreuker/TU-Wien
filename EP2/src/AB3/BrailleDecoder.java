...
public class BrailleDecoder implements Decoder {
    ...
    public char decodeBitmap(char[][] bitMap, char dotSymbol) {
        ...
        if (bitMap.length != BITMAP_HEIGHT || bitMap[0].length != BITMAP_WIDTH) return 0;
        byte decodedBraille = 0;
        int brailleIndex = 0;

        for (int i = 0; i < BITMAP_WIDTH; i++) {
            for (int j = 0; j < BITMAP_HEIGHT; j++) {
                if (bitMap[j][i] == dotSymbol) decodedBraille += (byte) (1 << brailleIndex++);
                else brailleIndex++;
            }
        }

        TreeNode decodedNode = decoderTree.getNode(decodedBraille);
        return decodedNode == null ? SPACE_SYMBOL : decodedNode.getSymbol();
    }
}
