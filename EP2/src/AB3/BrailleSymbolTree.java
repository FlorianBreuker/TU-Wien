...
public class BrailleSymbolTree implements Tree {
    ...
    public void addNode(char asciiCharacter) {
        // TODO: implementation
        if ((asciiCharacter < 'a' || asciiCharacter > 'z') && asciiCharacter != ' ') return;

        TreeNode currentNode = root;
        int byteLength = 6;
        byte currentByte = asciiCharacter != ' ' ? encoder.toBinary(asciiCharacter) : 0b000000;

        for (int j = 0; j < byteLength; j++) {
            if (currentByte % 2 == 0) {
                if (currentNode.getLeft() == null) currentNode.setLeft(new TreeNode());
                currentNode = currentNode.getLeft();
            } else {
                if (currentNode.getRight() == null) currentNode.setRight(new TreeNode());
