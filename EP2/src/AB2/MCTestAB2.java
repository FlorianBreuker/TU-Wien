    ...
    public static void main(String[] args) {
        checkAndPrint(
                ...
                new Question(
                        ...
                        // two times key "X", but k:X v:E is there first. get(X) return value "E", get(E) shouldn't happen bc "E" is no key but returns "null"
                        new Choice(false, "d.put(X, E); d.put(X, X); d.put(E, X);                                        (1B)"),
                        // two times key "X", but k:X v:E is there first. so probably true
                        new Choice(true, "d.put(X, E); d.put(d.get(E), d.get(X)); d.put(E, X);                          (1C)"),
                        // get(E) shouldn't exist so null
                        new Choice(false, "d.put(d.get(X), d.get(E)); d.put(X, X); d.put(E, E);                          (1D)"),
                        // k:null, v:null; k:X, v:X; k:E, v:E
                        new Choice(false, "d.put(X, X); d.put(d.get(X), d.get(E)); d.put(E, E);                          (1E)")
                        // k:X v:X; k:X, v:null; k:E, v:E
                ),
